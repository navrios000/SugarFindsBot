...
# Marcas frecuentes en productos de moda/streetwear.
KNOWN_BRANDS = [
    "ADIDAS",
    "ALEXANDER MCQUEEN",
    "AMIRI",
...
def detect_brand_from_name(name: str) -> str:
    """Intenta detectar una marca conocida directamente en el nombre."""

    if not name:
        return ""

    normalized = " ".join(name.upper().replace("_", " ").split())

    # Primero buscamos nombres completos para evitar coincidencias
    # parciales incorrectas.
...
async def detect_brand_from_image(image_url: str) -> str:
    """
    Analiza una imagen de producto con OpenAI Vision.

    Devuelve únicamente una marca reconocible.
    Si no puede identificarla con suficiente confianza, devuelve "".
    """
    ...
    try:
        ...
        brand = _extract_response_text(result)
        brand = brand.strip().upper()

        if not brand or brand == "NONE":
            return ""

        # Si la API devuelve una explicación en vez de solo la marca,
        # intentamos quedarnos con una marca conocida.
        detected = detect_brand_from_name(brand)

        if detected:
            return detected

        # También aceptamos una marca que no esté en nuestra lista,
        # siempre que la respuesta sea corta y parezca un nombre.
        if len(brand) <= 40 and "\n" not in brand:
            return brand
        ...
        logger.exception("Error detectando marca desde imagen")
        return ""


def _download_image(url: str) -> bytes:
    """Descarga una imagen con un User-Agent de navegador."""
...
def _extract_response_text(result: dict) -> str:
    """Extrae el texto de respuesta de la Responses API."""
